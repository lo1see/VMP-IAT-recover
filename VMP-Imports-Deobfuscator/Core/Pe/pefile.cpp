...
bool PEFile::load_from_process(uint32_t pid, const wchar_t* module_name)
{
	...
	ProcessMemory& sMemory = proc.memory();
	ProcessModules& sModules = proc.modules();

	ProcessCore& sCore = proc.core();

	proc.Detach();
	
	//auto sTargetModule = sModuleName == "" ? sModules.GetMainModule() : sModules.GetModule(std::wstring(sModuleName.begin(), sModuleName.end()));

	uintptr_t addr = module_name == NULL ? (*sModules.GetMainModule()).baseAddress : (*sModules.GetModule(module_name)).baseAddress;
	if (!addr)
	{
		return retValue;
	}
	...
	if(sMemory.Read(addr, data.size(), data.data()))
	{
		retValue = true;
...
void PEFile::add_iat_section(uint64_t base, uint32_t rva_oep, std::map<std::string, std::map<std::string, std::vector<uint64_t>>> AddedImports)
{
...
#else
	uint32_t newSize = thunk_count * sizeof(IMAGE_THUNK_DATA32) + dllName_count + funcName_count;
#endif // _WIN64
	...
	{
		if (!strcmp((char*)sec->Name, ".fixIat"))
		{
			newSec = sec;
			break;
		}
	...
	if (newSec == NULL)
	{
		append_section(".fixIat", newSize, IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_WRITE | IMAGE_SCN_MEM_EXECUTE, &newSec);
	}
...
#ifdef _WIN64
	PIMAGE_NT_HEADERS64 pNt = reinterpret_cast<PIMAGE_NT_HEADERS64>(data_ + pDos->e_lfanew);
#else
	PIMAGE_NT_HEADERS32 pNt = reinterpret_cast<PIMAGE_NT_HEADERS32>(data_ + pDos->e_lfanew);
#endif // _WIN64

	//PIMAGE_NT_HEADERS64 pNt64 = reinterpret_cast<PIMAGE_NT_HEADERS64>(data_ + pDos->e_lfanew);
	pNt->OptionalHeader.ImageBase = base;
	pNt->OptionalHeader.SizeOfImage += aligen(newSize);
	pNt->OptionalHeader.FileAlignment = 0x1000;
	pNt->OptionalHeader.DllCharacteristics = pNt->OptionalHeader.DllCharacteristics & (~IMAGE_DLLCHARACTERISTICS_DYNAMIC_BASE);

	if (rva_oep) {
		pNt->OptionalHeader.AddressOfEntryPoint = rva_oep;
	}

	// 重定向目录项中导入表的RVA
	pNt->OptionalHeader.DataDirectory[1].VirtualAddress = newSec->VirtualAddress;
	pNt->OptionalHeader.DataDirectory[1].Size = sizeof(IMAGE_IMPORT_DESCRIPTOR) * AddedImports.size() + sizeof(IMAGE_IMPORT_DESCRIPTOR);
...
#else
	PIMAGE_THUNK_DATA32 offset_thunk = (PIMAGE_THUNK_DATA32)(offset_descriptor + AddedImports.size() + 1); //+1是结尾的空描述符
#endif // _WIN64
...
bool PEFile::write_to_file(std::wstring filepath)
{
	FILE* f = NULL;
