...
bool Emulator::IsApiValid(std::unordered_map<DWORD_PTR, ApiInfo*>::iterator& ApiInfoIterator, std::uintptr_t Address)
{
	...
	{
		//(*ApiInfoIterator).second->module->modBaseAddr = (*it).second->module->modBaseAddr;
		ApiInfoIterator = it;
...
static void Emulator::TraceCallback(uc_engine* Unicorn, uint64_t Address, uint32_t Size, void* UserData)
{
...
#ifdef _WIN64
	err = uc_reg_read(Unicorn, UC_X86_REG_RSP, &RspValue);
#else
	err = uc_reg_read(Unicorn, UC_X86_REG_ESP, &RspValue);
#endif // _WIN64
	if (err != UC_ERR_OK)
	{
	...
	if (sIatPatchInfo.m_iCallIatMode != VMPCore::CALL_IAT_UNKNOWN)
	{
		...
		const WCHAR* wideFilename = (*ApiInfoIterator).second->module->getFilename();
		char charFilename[MAX_PATH] = { 0 };
		WideCharToMultiByte(CP_ACP, 0, wideFilename, -1, charFilename, MAX_PATH, nullptr, nullptr);
		strncpy(sIatPatchInfo.m_szModuleName, charFilename, sizeof(sIatPatchInfo.m_szModuleName) - 1);
...
bool Emulator::Init(void* PeBuffer)
{
	uc_err err;
#ifdef _WIN64
	err = uc_open(UC_ARCH_X86, UC_MODE_64, &Unicorn);
#else
	err = uc_open(UC_ARCH_X86, UC_MODE_32, &Unicorn);
#endif // _WIN64
	...
	std::unordered_map<uc_x86_reg, std::uintptr_t> unMapRegisters =
	{
		{UC_X86_REG_ESP, StackAddress + StackSize - sizeof(std::uintptr_t) * 100},
		{UC_X86_REG_EAX, 0x0},
		{UC_X86_REG_EBX, 0x0},
		{UC_X86_REG_ECX, 0x0},
		{UC_X86_REG_EDX, 0x0},
